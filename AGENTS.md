...
- `call.invoke` 现支持 descriptor-proven enum name/raw、owned FString 输入、经 `KismetTextLibrary` 在游戏线程转换的 FText 输入/输出，以及由当前 descriptor 完整描述的递归 value Struct 输入；`ParamFrame` 拥有 FString backing buffer 和逆序 destructor journal。生产类型源没有 witnessed `UEnum::Names` entry-table layout，所以真实目标枚举 descriptor 仍保持 unavailable；FText/函数返回的 UE allocation 尚未接入完整释放路径。`call.batch` 已由 Main/Core/Host/schema/TypeScript 装配：单 active coordinator 拥有最多 64 项、总 deadline、取消和 retained result；每项只复用 `FunctionCallCommandService::PrepareInvoke -> GameThreadExecutor -> CompleteInvoke`，snapshot 漂移稳定失败，不做 direct ProcessEvent/fallback。Weak/Soft、Array/Map/Set 与含这些字段的 Struct 输入仍未开放。
- Blueprint raw bytecode capture 已由 Main 接入 generation-bound runtime source：只有 `UFunction::Script` 通过 high-confidence multi-function witness、当前 Object/Type snapshot 完全匹配且 game-thread executor 可用时，`blueprint.bytecode` 才动态发布；捕获前后复核 exact FunctionHandle/TArray header/依赖，且 payload 必须以 `EX_EndOfScript` 结束。`blueprint.decompile` 仅在 immutable runtime dependencies 与本机 UE 源码 catalog 的 exact version marker、Name/FProperty 模型、outline-number 模式和 canonical math layout 一致时动态发布；parser 在首个不可靠 operand boundary fail closed，不按相近版本猜测。该 catalog 仍只是源码/合成证据，不是目标 Shipping 二进制 opcode/operand witness。Hook 现由 `ProcessEventHookOwner` 在 witnessed game thread 上消费当前 TypeSnapshot 的 Class CDO evidence，安装唯一 vtable patch 集，并以 callback barrier + RAII token 在 collector 之前恢复/排空；add/重新 enable 还会在游戏线程复核完整 FunctionHandle。`hook.monitor` 只在当前 Object/Type generation 已完整覆盖时动态发布；producer 开放 `fixed_metadata` 与 generation-bound `scalar_parameters`，后者只按 exact `ReflectedFunction` plan 在 callback 中有界复制 trivial scalar input/inout/output/return，worker 才校验 fingerprint 并解码。非平凡值和通用 `preencoded_payload` 稳定拒绝。Dump 现由 single-active coordinator 驱动 `SnapshotDumpWorker`：start 固定 exact immutable EngineContext/ObjectSnapshot/TypeSnapshot，Host 分配 bounded UUID output identity，Core 只写 `%LOCALAPPDATA%\UExplorer\Dumps` 下的 session/job 目录。SDK 输出是可直接包含的 typed snapshot header，包含字段/枚举/参数布局、padding/assert、metadata 和 ProcessEvent helper，但不是完整 Dumper-7 runtime framework；USMAP/Dumpspace/IDA 也只开放 bounded generator/consumer 能证明的内容。上述代码可达性均不等于真实 UE fixture 已验证。
...
- 按本轮“功能优先”口径，R5 六个领域的桌面主链路与可操作 UI 已完成装配；这不等于 R5 验收门通过。生产 UEnum entry table、Weak/Soft/container call input、完整 UE text allocation cleanup、Hook caller/condition/non-trivial capture、Dump Host reload persistence、FHitResult output 展示及全部真实目标 fixture 继续留在后续实现/验证清单，相关 issue 保持 `in_progress`。
...
- `Dumper/Services/FunctionCallCommandService.*`、`FunctionCallBatchCommandService.*`、`Dumper/Runtime/FunctionCallBatchCoordinator.*` 与 `ParamFrame.*`：单目标 exact ProcessEvent 及复用该路径的单 active bounded batch、精确参数 schema、owned frame、FString buffer、游戏线程 FText 转换、descriptor-backed recursive Struct、descriptor-proven enum codec、总 deadline/取消/逐项结果和逆序 destructor journal；生产 UEnum table、容器/引用型 Struct 输入与完整 UE allocation 释放仍未开放。
- `Dumper/Services/MemoryCommandService.*`：strict raw/typed/pointer-chain 命令；只允许 data write，不提供 executable/code-write fallback。
- `Dumper/Services/WatchCommandService.*`：exact Handle/generation watch CRUD、snapshot 与显式 pull drain；pull 队列不被 push 消费。
...
- `Dumper/Runtime/HookParameterCapture.*`、`Dumper/Services/HookCommandService.*` 与 `ProcessEventHookOwner.*`：exact generation/FunctionHandle command state、游戏线程 live admission、immutable enabled lookup/scalar plan、Class CDO vtable coverage、callback-side bounded raw copy、worker-only decode 与有序 restore/drain；只有当前 coverage 完整时 `hook.monitor` 才可用，目标 UE 行为仍未验证。
- `Dumper/Services/DomainEventPump.*`：独立 worker 排空 Watch/Hook push copy，做有界 JSON 序列化与 Named Pipe Event publication；不得进入游戏线程或 Hook callback，也不得消费 Watch pull/Hook retained log 查询状态。
- `Dumper/Services/DumpCommandService.*` 与 `SnapshotDumpWorker.*`：strict start/list/get/cancel、single-active owned worker、immutable snapshot pin、bounded SDK/USMAP/Dumpspace/IDA generation、cooperative cancel/deadline、`.partial` 无替换提交、size/SHA-256 复核和最后写入的 completion manifest。SDK 是 snapshot-backed typed header，包含字段/枚举/函数参数布局与调用辅助，但不包含完整对象查找 runtime；Host job 持久化和真实 UE artifact fixture 尚未完成。
...
- `Dumper/Services/WorldMutationCommandService.*`：单字段、generation-bound、exact-handle reflected setter，覆盖 world/relative location、rotation、scale；`FHitResult` output 使用 owned frame 的零初始化 slot 且不返回内容，调用后身份复核失败明确报告 mutation state unknown。

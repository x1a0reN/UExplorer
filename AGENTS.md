...
- `Dumper/Services/WorldTransformCommandService.*`：game-thread same-witness RootComponent stored transform 读取；只报告 `Relative*`、`bAbsolute*` 和 scalar precision，不计算 `ComponentToWorld`。transform mutation 仍使用 unavailable `world.mutate`。

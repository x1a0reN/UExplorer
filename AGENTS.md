...
- 已知仍未关闭的事实包括：默认 `AllocConsole` 与 F6 路径、Dumper 配置的 current-directory/global-path 行为、剩余 `Off::*/Settings::*` 和 legacy domain monolith、Hook ProcessEvent producer、Watch push producer、Dump generator worker/output-root/snapshot pin、生产 UEnum/Blueprint witnesses、call.batch 真实 ProcessEvent/取消/超时 fixture、前端 session/query 状态重构以及真实 UE/GC/卸载/性能 fixture。

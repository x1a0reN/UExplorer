...
- 当前 Host 已实现 status、immutable snapshot-backed Object/Type 集合查询、exact-path Class/Struct/Enum/Function 详情、exact stable-handle `objects.property.read`、R5.2 单目标 `call.invoke`，以及 R5.3 的只读 `world.inspect/world.levels/world.actors.list`。Core 会从一代稳定 Object Snapshot 自动采集 witnessed ReflectionLayout，再由生产 `ObjectSnapshotTypeCandidateSource` 从 exact Object/Reflection generation 采集完整类型/函数结构和 flat property descriptor；World capture 在 Worker 预筛 Actor/Level class candidates，并在共享 PostRender scheduler 上通过 `GWorld`、AActor typed-outer chain 与 witnessed `ULevel.OwningWorld` 分帧构建和复核，排序、计数、store validation 与 publication 留在 Worker。函数调用只接受 exact ObjectHandle、FunctionHandle、TypeSnapshot generation、完整显示路径和按参数名/反射 kind 编码的参数；Core 用 owned `ParamFrame` 在 witnessed game thread 再验证全部句柄后调用 ProcessEvent。当前输入只开放 bool、整数、float/double、UObject/null，输出还可解码 FName；static 调用由前端显式取得 CDO handle。FString/FText/Weak/Soft/Struct/Array/Map/Set 的调用生命周期、enum、batch job、真实 UE round-trip，以及 property write、Memory、World detail/shortcut/transform、Watch、Hook、Blueprint、Dump 仍待实现，不得回退旧实现。
...
- `Dumper/Services/WorldCommandService.*`：worker-only immutable WorldSnapshot 查询；当前只开放 inspect、Level page 与 Actor page，cursor 绑定 generation/query；Actor detail/components/shortcuts 使用 unavailable `world.details`，transform 使用 unavailable `world.mutate`。

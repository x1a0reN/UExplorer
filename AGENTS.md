...
- 当前 Host 已实现 status、immutable snapshot-backed Object/Type 集合查询、exact-path Class/Struct/Enum/Function 详情、exact stable-handle `objects.property.read`、R5.2 单目标 `call.invoke`，以及 R5.3 的 World 查询与 `world.actor.transform.get`。Core 会从一代稳定 Object Snapshot 采集 witnessed ReflectionLayout 和完整 TypeSnapshot，canonical FVector/FRotator 只接受 exact CoreUObject path、语义字段、float/double kind、offset、size 与 alignment，不按版本或 LWC 猜测。World capture 分帧构建并复核 immutable World 关系；独立的 `WorldTransformCommandService` 在一个 owned game-thread work 中固定 exact World/Object/Type/codec generation，重验 Actor/RootComponent Handle，从同一稳定 byte witness 读取三个 `Relative*` 和三个 `bAbsolute*`，并在前后 witness 之间通过 exact `K2_GetActorLocation`、`K2_GetActorRotation`、`GetActorScale3D` 与 owned return frame取得 computed Actor world transform。React 将 stored 与 computed 值显式分开；getter capability/metadata 缺失时 computed 部分返回稳定 unavailable，不猜测 `ComponentToWorld`。函数调用输入现开放 bool、整数、float/double、UObject/null，以及 descriptor-proven canonical FVector/FRotator；这两个 math struct 的 output/return 也可从 owned frame 解码，任意 Struct 仍不开放。UE 4.21-5.7 源码证明 `SetActorScale3D(FVector)`/`SetRelativeScale3D(FVector)` 不携带复杂 output，因此可经 exact `call.invoke` 路径执行；完整 `world.actor.transform.update` 仍 unavailable，location/rotation setter 所需 FHitResult 生命周期尚未实现。FString/FText/Weak/Soft/任意 Struct/Array/Map/Set 输入、enum、batch、真实 UE round-trip，以及 property write、Memory、Watch、Hook、Blueprint、Dump 仍待实现，不得回退旧实现。computed transform 当前只有代码证据；Wandering Sword 目录没有可启动游戏 `.exe`，所有 profile 仍为 `Not supported`。
...
- `Dumper/Services/FunctionCallCommandService.*` 与 `Dumper/Runtime/ParamFrame.*`：单目标 ProcessEvent 调用、精确参数 schema、owned frame、canonical FVector/FRotator input/output 和 output decode；任意 Struct/复杂生命周期类型与 batch 未开放。
...
- `Dumper/Services/WorldTransformCommandService.*`：game-thread same-witness RootComponent stored transform 读取，并在同一 owned work 内以 exact reflected Actor getter + owned return frame取得 computed world location/rotation/scale；两类值显式分离，computed 能力不可用时返回稳定原因。transform mutation 仍使用 unavailable `world.mutate`。

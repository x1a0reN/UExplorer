...
- `Dumper/Services/HookCommandService.*` 与 `ProcessEventHookOwner.*`：exact generation/FunctionHandle command state、游戏线程 live admission、immutable enabled lookup、Class CDO vtable coverage、bounded callback publication 与有序 restore/drain；只有当前 coverage 完整时 `hook.monitor` 才可用，目标 UE 行为仍未验证。

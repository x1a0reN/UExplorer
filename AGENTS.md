...
- 当前 Host 已实现 status、immutable snapshot-backed Object/Type 集合查询、exact-path Class/Struct/Enum/Function 详情、exact stable-handle `objects.property.read`、R5.2 单目标 `call.invoke`，以及 R5.3 的只读 `world.inspect/world.levels/world.actors.list/world.shortcuts/world.actor.get/world.actor.components`。Core 会从一代稳定 Object Snapshot 自动采集 witnessed ReflectionLayout，再由生产 `ObjectSnapshotTypeCandidateSource` 从 exact Object/Reflection generation 采集完整类型/函数结构、flat descriptor、单层 `Array<flat>` descriptor，以及 exact `/Script/CoreUObject.Vector`/`Rotator` 的三字段 float/double descriptor；`FRotator` 只接受 `Pitch/Yaw/Roll`，不按版本、全局 LWC 或 size 猜测。Windows x64 PropertyCodec 使用明确的 16-byte ScriptArray profile，并对 struct 做整值稳定快照后再解码。World capture 在 Worker 预筛 Actor/Level/ActorComponent candidates，并在共享 PostRender scheduler 上通过 `GWorld`、Actor/Component typed-outer chain、witnessed `ULevel.OwningWorld`、`AActor.RootComponent`、`UWorld.AuthorityGameMode/GameState` 和 exact `OwningGameInstance -> LocalPlayers[0] -> PlayerController -> Pawn` 分帧构建和复核，排序、计数、store validation 与 publication 留在 Worker。Actor detail/components 必须使用 exact ActorHandle + WorldSnapshot generation，组件 cursor 还绑定 Actor 身份；local-player metadata 不完整时 PlayerController/Pawn shortcut 显式 unavailable，不扫描其他数组项或全局对象。React WorldBrowser 可通过三次 exact property read 展示 RootComponent 存储的 `RelativeLocation/RelativeRotation/RelativeScale3D`，但这三次读取不保证同一帧，尚未纳入 `bAbsolute*`，也不是 computed world transform；setter/write 继续 unavailable。函数调用只接受 exact ObjectHandle、FunctionHandle、TypeSnapshot generation、完整显示路径和按参数名/反射 kind 编码的参数；Core 用 owned `ParamFrame` 在 witnessed game thread 再验证全部句柄后调用 ProcessEvent。当前输入只开放 bool、整数、float/double、UObject/null，输出还可解码 FName 及具备 descriptor 的结构；static 调用由前端显式取得 CDO handle。FString/FText/Weak/Soft/Struct/Array/Map/Set 的调用输入生命周期、enum、batch job、真实 UE round-trip，以及 property write、Memory、同帧 World transform、Watch、Hook、Blueprint、Dump 仍待实现，不得回退旧实现。
...
- `Dumper/Services/WorldCommandService.*`：worker-only immutable WorldSnapshot 查询；开放 inspect、Level/Actor/Component page、exact-handle Actor detail 与显式状态 shortcuts。所有 cursor 绑定 generation/query，Component cursor 额外绑定 ActorHandle；transform 使用 unavailable `world.mutate`。

...
void CppGenerator::GenerateVTHookFile()
{
	StreamType VTHookFile(MainFolder / "VTHook.hpp");

	VTHookFile << "\xEF\xBB\xBF";
	VTHookFile << R"(#pragma once
...
public:
	...
	// Install / Remove / GetOriginal / IsInstalled
	VTHookFile << R"(	template<typename FuncType>
...
void CppGenerator::GenerateVSProject(const std::vector<std::string>& AllHppFiles, const std::vector<std::string>& AllCppFiles)
{
	// Sanitize GameName for use as filename and in XML content
	auto SanitizeFileName = [](const std::string& Name) -> std::string
	{
		...
		for (char c : Name)
		{
			// Replace characters invalid in Windows filenames and XML
			if (c == '<' || c == '>' || c == ':' || c == '"' || c == '/'
				|| c == '\\' || c == '|' || c == '?' || c == '*' || c == '&')
				Result += '_';
			else
	...
	auto XmlEscape = [](const std::string& Text) -> std::string
	{
		std::string Result;
	...
	// Generate a stable GUID from GameName using FNV-1a hash
	auto GenerateGuid = [](const std::string& Seed) -> std::string
	{
		...
		uint32_t A = static_cast<uint32_t>(Hash >> 32);
		uint16_t B = static_cast<uint16_t>(Hash >> 16);
		uint16_t C = static_cast<uint16_t>(Hash) | 0x4000; // Version 4
		uint16_t D = static_cast<uint16_t>(Hash >> 48) | 0x8000; // Variant 1
		uint32_t E = static_cast<uint32_t>(Hash);

		return std::format("{{{:08X}-{:04X}-{:04X}-{:04X}-{:08X}{:04X}}}",
			A, B, C & 0x4FFF, D & 0xBFFF, E, static_cast<uint16_t>(Hash >> 8));
	};

	const std::string RawGameName = Settings::Generator::GameName.empty() ? "GameSDK" : Settings::Generator::GameName;
	const std::string GameName = SanitizeFileName(RawGameName);
	const std::string XmlGameName = XmlEscape(RawGameName);
	const std::string InjectGuid = GenerateGuid("Dumper7-Inject-" + RawGameName);
	const std::string ProxyGuid = GenerateGuid("Dumper7-Proxy-" + RawGameName);

	const fs::path InjectDir = MainFolder / "Inject";
	const fs::path ProxyDir = MainFolder / "Proxy";
	...
	// -- Inject/.slnx --
	{
		...
		Slnx << R"(<Solution>
  <Project Path=")" << GameName << R"(_Inject.vcxproj" />
</Solution>
)";
	}
...
DWORD MainThread(HMODULE Module)
{
	...
	std::cout << "[SDK] DLL Loaded. Press DELETE to unload.\n" << std::endl;
	...
	// Cleanup: unhook, close console, unload DLL
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved)
{
	...
	// -- Proxy/.slnx --
	{
		...
		Slnx << R"(<Solution>
  <Project Path=")" << GameName << R"(_Proxy.vcxproj" />
</Solution>
)";
	}
	...
	// -- Proxy/Main.cpp --
	{
		...
		// Part 1: Includes and version.dll export forwarding
		ProxyCpp << R"(#include <Windows.h>
#include <iostream>
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved)
{
	...
	{
		StreamType Vcxproj(Dir / (ProjName + ".vcxproj"));
...
<Project DefaultTargets="Build" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <PropertyGroup Label="Globals">
    ...
    <RootNamespace>)" << XmlGameName << R"(</RootNamespace>
  ...
  <Import Project="$(VCTargetsPath)\Microsoft.Cpp.Default.props" />
		...
		Vcxproj << R"(  <PropertyGroup Condition="'$(Configuration)|$(Platform)'=='Release|x64'" Label="Configuration">
  ...
  <Import Project="$(VCTargetsPath)\Microsoft.Cpp.props" />
		...
		// TargetName override (e.g. "version" for proxy DLL)
		if (!TargetNameOverride.empty())
		{
			Vcxproj << "  <PropertyGroup>\n";
			Vcxproj << "    <TargetName>" << TargetNameOverride << "</TargetName>\n";
			Vcxproj << "  </PropertyGroup>\n\n";
		}
...
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  ...
  <ItemGroup>
	...
	WriteVcxproj(InjectDir, GameName + "_Inject", InjectGuid, "");
	WriteVcxproj(ProxyDir, GameName + "_Proxy", ProxyGuid, "version");
}
...
void CppGenerator::Generate()
{
	...
	// File lists for VS project generation
	std::vector<std::string> AllHppFiles;
	std::vector<std::string> AllCppFiles;

	AllHppFiles.push_back("Basic.hpp");
	AllCppFiles.push_back("Basic.cpp");
	...
	for (PackageInfoHandle Package : PackageManager::IterateOverPackageInfos())
	{
		...
		if (Package.HasClasses())
		{
			...
			AllHppFiles.push_back(FileName + "_classes.hpp");
		}
		...
		if (Package.HasStructs() || Package.HasEnums())
		{
			...
			AllHppFiles.push_back(FileName + "_structs.hpp");
		}
		...
		if (Package.HasParameterStructs())
		{
			...
			AllHppFiles.push_back(FileName + "_parameters.hpp");
		}
		...
		if (Package.HasFunctions())
		{
			...
			AllCppFiles.push_back(FileName + "_functions.cpp");
		}
	...
	// Generate VS project files (.slnx, .vcxproj, Main.cpp)
	GenerateVSProject(AllHppFiles, AllCppFiles);
}

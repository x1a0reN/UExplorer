...
void RegisterWorldRoutes(HttpServer& server)
{
	...
	server.Post("/api/v1/world/actors/:index/transform", [](const HttpRequest&) -> HttpResponse {
		return { 409, "application/json",
			MakeError("ACTOR_TRANSFORM_WRITE_DISABLED: a verified game-thread UE setter is required") };
	});

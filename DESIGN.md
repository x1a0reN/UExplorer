...
| R5 领域正确性 | 进行中（R5.1 property read、R5.2 单目标 call、R5.3 immutable World detail/components/partial shortcuts 代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor 与 baseline PropertyCodec；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame` 并在 witnessed game thread 调用 ProcessEvent；`WorldSnapshotCapture` 以 exact Object/Type generation 预筛 Actor/Level/ActorComponent，在共享 PostRender 预算内解析并复核 GWorld、typed outer、`ULevel.OwningWorld`、`AActor.RootComponent` 与 `UWorld.AuthorityGameMode/GameState`，Worker 完成排序、精确计数和 publication；`WorldCommandService` 暴露 inspect/Level/Actor/Component cursor page、exact-handle Actor detail 与显式状态 shortcuts；Host 与 React 使用同一 DomainService/Named Pipe 主链路 | Wandering Sword 可执行文件/干净基线恢复及真实运行证据；World streaming/GC/目标规模；`UGameInstance.LocalPlayers`/PlayerController/Pawn shortcut；FVector/FRotator/LWC transform codec 与 UE setter；复杂调用生命周期、enum input、batch job、真实 function round-trip、property write，以及 Memory/Watch/Hook/Blueprint/Dump |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“已由真实 UE 验证可用”的既有功能包括：R5.2 `call.invoke`、R5.3 全部 `world.*` 只读命令、Watch/Hook producer、Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、Memory、LocalPlayer/Pawn shortcut、World transform、Blueprint/Dump 等尚未迁移或仅有 synthetic evidence 的领域能力、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。
...
- `UGameInstance.LocalPlayers` 在 UE4/UE5 都是 `TArray`，当前 World capture 尚未拥有经该 TypeSnapshot generation 验证的 array profile/order traversal，因此 PlayerController/Pawn shortcut 返回 `WORLD_LOCAL_PLAYER_CHAIN_UNAVAILABLE`，不再沿用 legacy“全局第一个 class substring”。Transform detail 返回 `WORLD_TRANSFORM_CODEC_UNAVAILABLE`；没有按 size/LWC 全局开关猜 `FVector/FRotator`，也没有开放内存写入。
- 本轮 Core Release、CoreHarness、Host route test、React lint/test/build 与协议 JSON 解析覆盖 immutable relation、exact handle/generation、Actor-bound component continuation、stale serial 拒绝和显式 unavailable state；未运行真实 UE、streaming、GC、LWC 或 setter fixture，因此 WORLD-002/WORLD-004/WORLD-005 仍保持进行中，WORLD-006 保持 open，支持矩阵不变。

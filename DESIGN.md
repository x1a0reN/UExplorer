...
| R2 CoreRuntime/能力模型 | 进行中 | CoreRuntime 状态机、加密随机 session 与 request lease；一次性发布的 immutable EngineContext/identity/name-layout context；offset validation report；只依赖冻结 profile 与 SafeMemory 的 NamePool/Chunked NameArray 严格名称 codec；依赖式 CapabilityRegistry；Runtime-owned GameThreadExecutor、IGameThreadPump/PostRender backend 与 thread/tick/stall 诊断；ShutdownCoordinator；集中式 SafeMemory 与可执行页写策略；稳定 Object/FunctionHandle 契约、生产 identity source 与执行点重验证；`EngineFacade` 单一引擎边界、原子 immutable Snapshot store、生产 object/path metadata source 与 capture/validate/publish 三阶段有界 producer；Snapshot producer 已通过独占且可排空的 frame client 接入 PostRender；transport-neutral CoreCommandService 已承载 status 和 handle issue；VTable Hook RAII owner 与 callback quiet drain；危险 UObject 属性写及 index-only 调用入口已关闭 | identity/name/snapshot 的真实目标与 GC churn fixture、其余领域 command、剩余 Off/Settings 迁移 |
...
R2 尚未完成：`FUObjectItem` identity/name/snapshot 的真实目标 profile 与 GC churn/大型对象表 fixture、其余 Object/Class/Enum/Memory/World/Call/Blueprint/Dump 领域命令、Hook 热路径有界 collector/最后订阅卸载，以及剩余直接读取 `Off::*`/`Settings::*` 或遍历 live UE 容器的模块仍待迁移；这些完成前不会把 R2 标记为完成。

...
| R5 领域正确性 | 进行中（R5.1 property read 与 R5.2 单目标 call 代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor 与 baseline PropertyCodec；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame`，只编码 trivial scalar/UObject 输入，在 witnessed game thread 重新验证目标、函数 owner/signature 与对象参数后调用 ProcessEvent，并解码 out/inout/return；Host 与 React 已接入同一 `call.invoke`，static 使用显式 CDO handle，协议不暴露线程选择 | Wandering Sword 真实运行证据；FString/FText/Weak/Soft/Struct/container 调用生命周期；enum input、batch job、真实 function round-trip、property write、GC/目标规模，以及 Memory/World/Watch/Hook/Blueprint/Dump |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“已由真实 UE 验证可用”的既有功能包括：R5.2 `call.invoke`、Watch/Hook producer、Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、Memory/World/Blueprint/Dump 等尚未迁移的领域命令、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。

...
| R5 领域正确性 | 进行中（R5.1 查询、Codec 基础、反射与类型结构采集完成） | Object/Type 列表查询已复用 Host SnapshotIndex；PropertyCodec 具备有界值模型；ReflectionLayout 验证完整字段集和 UStruct 尺寸/对齐；生产 `ObjectSnapshotReflectionCandidateSource` 从单代 immutable Object Snapshot + SafeMemory 生成 UProperty/FProperty witness；生产 `ObjectSnapshotTypeCandidateSource` 再从 exact Object/Reflection generation 采集完整类型/函数结构、direct property/parameter、super/CDO 与 witnessed native exec；两条 capture 均由 Main 接入共享 PostRender scheduler，分帧复核、精确依赖封口并由 Worker publication/回收；Object Snapshot 逐条校验、分段存储并把旧代/失败工作集移交 Worker 回收；合成 C++ fixture 已通过 | 真实 UE profile 的 reflection/type 采集证据、生产 PropertyCodec profile/capture、property descriptor/enum/bytecode 元数据、Named Pipe/Host 详情 command、GC/目标规模，以及 Memory/Call/World/Watch/Hook/Blueprint/Dump 逐项验证 |
...
- `engine.type_snapshot` 是单独的数据 readiness capability；`types.inspect` 仍因 Core/Named Pipe/Host 详情 command 尚未注册而保持 `TYPE_COMMAND_NOT_IMPLEMENTED`。生产 `ObjectSnapshotTypeCandidateSource` 与 Main 调度已接线，满足完整结构覆盖时可发布该 capability；但当前 property descriptor、enum layout 和 Blueprint bytecode 没有 witnessed 生产布局，必须逐项记录为 `unavailable`，不能伪装为完整类型详情。合成证据只证明 fail-closed 边界，不宣称任一真实游戏版本可用。
...
- 当前结论只到“生产 reflection/type 路径已接线并通过 UProperty 合成结构图；reflection 另通过 FProperty 合成内存图”。真实 UE 4.26/4.27/5.x 的 fixture、GC/加载态与目标规模仍未执行，因此支持矩阵不变；`engine.property_codec` 仍缺生产 profile/capture，`types.inspect` 仍缺详情 command，旧 `Off::InitReflection()` 不是 fallback。

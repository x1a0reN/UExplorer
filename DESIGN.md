...
- `WatchScheduler` 保留原 `watch.events.drain` pull ring，并为相同 owned event 建立独立有界 push ring；两个 ring 各自 coalesce/evict/累计 drop，publisher 排空 push 时不会消费 pull。`HookCommandService` 在 worker drain collector、更新 retained log 后，将 fixed-metadata record 复制到独立有界 push ring；ProcessEvent callback 仍只进入预分配 collector，不做 JSON、Pipe I/O、服务锁或 Host 等待。
...
- lifecycle 依赖是 BeginStopping/关闭 Pipe admission/settle requests -> 恢复并排空 ProcessEvent producer -> 停止 Watch producer与 collector -> 排空 event worker -> 释放 Pipe/服务。Pipe 已关闭后的最终 drain 会明确计为 publish drop，不把未送达事件伪装成功；event worker 未退出时不释放其 Watch/Hook/Pipe 引用。`watch.events.push` 与 `hook.events.push` 只是 status capability key，只反映该 owned worker 及对应 producer 当前事实，不是可请求 operation，也不替代参数 capture。
- 本检查点只证明代码、schema、构建与既有通用 transport chain；没有运行 Wandering Sword/真实 UE 的 Watch mutation、ProcessEvent rate、nested call、disconnect/replay、慢消费者、drop/coalesce 或卸载 fixture。Hook 仍只支持 fixed metadata，不包含参数/return frame。NET-015、WATCH-003、HOOK-003、HOOK-008 保持 `in_progress`，所有 UE profile 继续 `Not supported`。

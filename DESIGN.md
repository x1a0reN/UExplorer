...
R2 尚未完成：`FUObjectItem` identity source 的真实目标 profile fixture、Handle 到领域执行命令的接入、Hook 热路径有界 collector/最后订阅卸载、领域 command 边界和剩余直接读取 `Off::*`/`Settings::*` 或遍历 live UE 容器的模块仍待迁移；这些完成前不会把 R2 标记为完成。

...
| R5 领域正确性 | 进行中（R5.1 property read、R5.2 单目标 call、R5.3 immutable World detail/components/local-player shortcuts 与存储 Relative* 读取代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor、单层 `Array<flat>` descriptor、exact FVector/FRotator float/double descriptor 与 Windows x64 ScriptArray PropertyCodec；struct 从整值稳定快照解码；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame` 并在 witnessed game thread 调用 ProcessEvent；`WorldSnapshotCapture` 以 exact Object/Type generation 预筛 Actor/Level/ActorComponent，在共享 PostRender 预算内解析并复核 GWorld、typed outer、`ULevel.OwningWorld`、`AActor.RootComponent`、`UWorld.AuthorityGameMode/GameState` 及 `OwningGameInstance -> LocalPlayers[0] -> PlayerController -> Pawn`，Worker 完成排序、精确计数和 publication；`WorldCommandService` 暴露 inspect/Level/Actor/Component cursor page、exact-handle Actor detail 与显式状态 shortcuts；WorldBrowser 通过 exact property reads 展示 RootComponent 存储的 RelativeLocation/Rotation/Scale；Host 与 React 使用同一 DomainService/Named Pipe 主链路 | Wandering Sword 可执行文件/干净基线恢复及真实运行证据；World streaming/GC/目标规模；同帧 aggregate、`bAbsolute*`、computed world transform 与 UE setter；复杂调用生命周期、enum input、batch job、真实 function round-trip、property write，以及 Memory/Watch/Hook/Blueprint/Dump |

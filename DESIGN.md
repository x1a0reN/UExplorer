...
| R5 领域正确性 | 进行中（R5.1 property read、R5.2 单目标 call、R5.3 immutable World 与 same-frame stored transform 代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor、单层 `Array<flat>` descriptor、exact FVector/FRotator float/double descriptor 与 Windows x64 ScriptArray PropertyCodec；struct 从整值稳定快照解码；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame` 并在 witnessed game thread 调用 ProcessEvent；`WorldSnapshotCapture` 构建并复核 immutable World 关系；`WorldCommandService` 提供 worker-only 查询；`WorldTransformCommandService` 在一个 owned game-thread work 中以同一稳定 byte witness 读取 RootComponent 三个 `Relative*` 与三个 `bAbsolute*`，Host/React 通过单个 `world.actor.transform.get` 展示 precision 和 storage-space 语义 | Wandering Sword 可执行文件/干净基线恢复及真实运行证据；World streaming/GC/目标规模；computed `ComponentToWorld` 与 witnessed UE setter；Struct/FHitResult 等复杂调用生命周期、enum input、batch job、真实 function round-trip、property write，以及 Memory/Watch/Hook/Blueprint/Dump |
...
- `D:\Projects\UnrealEngine` 的 4.21-5.7 `SceneComponent.h` 显示 location/rotation 的 reflected setter 均携带 `FHitResult&` output，而 scale setter接收 FVector。当前 `ParamFrame` 尚不支持 canonical Struct input 或 FHitResult output lifetime，因此 `world.actor.transform.update` 继续由 `world.mutate` 明确拒绝；没有恢复 raw field write，也没有用零初始化复杂 output 冒充生命周期支持。

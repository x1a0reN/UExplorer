...
| R6 前端状态重构 | 功能代码完成；完整验收未执行 | contracts/transport/services/session/features 分层；统一 session/query/event 状态；BigInt 地址；Objects/Functions/World/Memory 实际交互；五层连接状态；真实设置项；frontend production 与 post-push Core Release build 成功 | lint/unit/component/真实 UE 交互验证按本轮要求未执行，留待 R7 前统一收口 |
...
- 旧 Hook monitoring 含锁、JSON 和网络热路径，因此已从 release 隔离；本条记录的是 R1 关闭决策。R5 的有界 collector 与生产 ProcessEvent owner 已由 0.26 替代旧实现，worker-side Pipe/Tauri push 由 0.28 覆盖，目标 fixture 仍待完成。
...
- Host 为 Core execution timeout 增加 1 秒 settlement grace，并在 request frame 完整写入后按实际发送完成时刻重新绑定本地 settlement deadline，避免写入延迟或正常 late terminal response 让 Host 先于 Core 丢弃结果。同步 caller completion wait 现在严格长于该 settlement window，保证正常 deadline 由 I/O worker 发布唯一 `DeadlineExpired`，不会在同一时刻竞态降级成 generic `WaitTimeout`；HoldRequests Named Pipe fixture 会消费并校验 Request/Cancel frame、只抑制对应 Response，避免 split/coalesced read 改变测试语义。响应断线后 mutation 结果仍不可查询，后续仍须增加 operation id/result query；在此之前 mutation 的 transport failure 不得盲目重试。
...
### 0.25 R5.5 witnessed Blueprint capture 与 source-catalog profile
...
- Main 仅在 EngineContext 发布上述 validated Script report 时构造 runtime source；动态 `blueprint.bytecode` capability 还要求当前 Object/Type generation 与 game-thread executor 同时可用。Core/Host/schema/TypeScript 已有的 exact FunctionHandle 请求因此可进入 raw hex capture。
- 同一个 runtime source 现在按本机受审计 UE 源码清单发布显式 profile catalog：4.21、4.24-4.27、5.0-5.4、5.6、5.7 各自记录 raw opcode transition、Windows x64 pointer、官方 source candidate 的 32-bit `CodeSkipSizeType`、12-byte `FScriptName`、FVector/FRotator/FTransform component width 与 parser limits；没有本机源码的 4.22、4.23、5.5 以及未知 marker 保持 unavailable。`FScriptName` 不再被简化成 `FName`：profile 明确包含 comparison/display/number 三段，并按运行时 name profile 区分 inline number 与 UE5 outline number；outline padding 非零会在可靠边界失败。
- profile publication 不是只看 `GameVersion`：独立 PE release-marker probe 必须与 immutable EngineContext 完全一致，当前 name storage、FProperty mode、outline-number mode、LWC flag 以及 TypeSnapshot 中 canonical `/Script/CoreUObject.Vector`/`Rotator` 三字段 kind/offset/size/alignment 必须同时匹配 source candidate，且前后 snapshot `shared_ptr` 不能漂移。profile ID 固化 marker family、source build 与 FScriptName number mode；`status.inspect/status.engine` 只在这些门仍成立时返回该 ID，Main 才动态开放 `blueprint.decompile`，不存在相邻版本、pointer width、name layout 或旧 parser enum fallback。
- bounded parser 已补齐 source inventory 中的 `NothingInt32`、`BitFieldConst`、`Vector3fConst` 和 AutoRTFM 三个 token，按 transaction id/mode 消费嵌套 StopTransact，并修正 multicast call 必须先消费 delegate expression、InlineEvent 必须消费完整 FScriptName。Functions 页面从 Core status 获取 active profile ID，并始终显示 `status/coverage/bytes/unknown/end_of_script/first_error`，因此 incomplete 结果不会只以 pseudocode 伪装成完整反编译。
- 该切片只完成生产代码、源码差异审计与静态/合成边界，不代表目标 Blueprint 已验证。尚未运行 Wandering Sword/真实 UE 的 Script offset、代表性 opcode/operand、Blueprint function、GC/snapshot churn、超时、热重载或卸载 fixture；自定义引擎仍可能改变 `SCRIPT_LIMIT_BYTECODE_TO_64KB`、opcode 或序列化布局，必须由 R7 target fixture 证明 catalog candidate 实际适用。BP-001/BP-002/BP-003 保持 `in_progress`，所有 UE profile 继续 `Not supported`。
...
- owner 为每个唯一 vtable slot 保留独立原始 ProcessEvent 指针和 RAII `VTableHookToken`，因此允许不同 native class override，而不是把所有 slot 错当成同一 original。patch 前发布 immutable slot dispatch；活动 owner 使用原子 `shared_ptr` 发布，回调在读取状态后持有强所有权并通过 SEH-bounded object-vtable/function-address 查表，只向预分配 `HookEventCollector` 写入已成功配对的 enter/exit fixed metadata 或由 exact TypeSnapshot plan 限定的 trivial scalar raw copy；回调不做参数解码、JSON/Pipe I/O、服务 mutex 或 Host 等待。Main 的生产配置只开放 `fixed_metadata` 与 `scalar_parameters`，继续明确拒绝未见证的通用 `preencoded_payload` producer。
...
- 关闭顺序现在是 Pipe/request settlement -> ProcessEvent maintenance drain -> 逆序恢复全部 slot -> callback quiet drain -> Watch/collector stop -> DomainEventPump drain -> frame clients/PostRender/EngineFacade。restore、maintenance、callback 或 event-pump drain 任一失败都会阻止后续破坏性 teardown 与 DLL unload；初始化失败路径同样先停 event worker，再释放 Pipe。旧 `API/HookApi.cpp` 仍只作历史证据，release producer 不接 HTTP/SSE/legacy globals。
- 当前证据仅为 Core Release、`/W4 /WX` CoreHarness 构建/合成 harness 与静态 contract；没有运行 Wandering Sword/真实 UE 的 Class CDO coverage、并发 ProcessEvent、热重载、GC、event rate、third-party hook chaining、slot restore 或卸载 fixture。Pipe/Tauri push 的代码链由 0.28 覆盖，但不是目标 fixture。HOOK-001/002/003/005/006/008 只增加 code evidence并保持 `in_progress`，所有 UE profile 保持 `Not supported`。
...
- 四条生成路径都只消费 pinned immutable TypeSnapshot 并受 type/member/artifact/byte 上限约束。SDK 现生成 `uexplorer.snapshot-sdk.v2` 单头文件：包含唯一化 C++ 类型名、枚举、按 offset 排列的 typed property、显式 padding/size/alignment/offsetof assert、父类布局基块、bool bit accessor、FName/FString/TArray/weak/soft/container 基础表示、每个 UFunction 的参数结构、metadata/native RVA 与 ProcessEvent invoke helper。当前快照没有足够 descriptor 的成员仍以保留准确大小的命名 storage 表示，不伪造未知语义；它是可直接包含和访问的 snapshot SDK，但还不是带对象查找/runtime framework 的完整 Dumper-7 SDK。USMAP 写 v4 uncompressed container，并由独立 bounded parser 复核结构；缺 enum entry table、完整 property descriptor 或可表示类型时 fail closed。Dumpspace 输出五个 bounded JSON document 并重新严格解析；IDA 输出经过符号清理的 native-function RVA Python records，并经过 bounded structure consumer。
...
本轮按功能优先完成 typed SDK 代码路径，仅运行 Core Release 编译检查；没有运行 Wandering Sword/真实 UE，也没有执行 artifact 外部消费者、压力、性能、GC 或卸载验证，因此不把这些项目记为已验证。

### 0.28 R5.4 bounded Watch/Hook push event chain

- `WatchScheduler` 保留原 `watch.events.drain` pull ring，并为相同 owned event 建立独立有界 push ring；两个 ring 各自 coalesce/evict/累计 drop，publisher 排空 push 时不会消费 pull。`HookCommandService` 在 worker drain collector、更新 retained log 后，将 fixed-metadata 或 bounded scalar payload record 复制到独立有界 push ring；ProcessEvent callback 仍只进入预分配 collector，不做解码、JSON、Pipe I/O、服务锁或 Host 等待。
- 新增 owned `DomainEventPump` worker。它固定同一 session/context 的 Watch、Hook 和 `NamedPipeRpcServer`，每批有明确 event/data 上限，在非游戏线程完成 Base64/hex/JSON 序列化，并发布四种 `watch.*` 与 paired `hook.process_event_enter/exit` Event。过大值只在 push envelope 中以稳定 omission code 省略，pull history/retained Hook log 不受影响；push ring、publisher、Pipe、Host EventHub/subscriber 的 loss counter 分层外显。React 使用 schema 对应的 typed guard，在调用方持有的 Tauri Channel 上增量更新 Watch history/list 与 Hook hit/log，不再为每次事件全量 refetch。
- lifecycle 依赖是 BeginStopping/关闭 Pipe admission/settle requests -> 恢复并排空 ProcessEvent producer -> 停止 Watch producer与 collector -> 排空 event worker -> 释放 Pipe/服务。Pipe 已关闭后的最终 drain 会明确计为 publish drop，不把未送达事件伪装成功；event worker 未退出时不释放其 Watch/Hook/Pipe 引用。`watch.events.push` 与 `hook.events.push` 只是 status capability key，只反映该 owned worker 及对应 producer 当前事实，不是可请求 operation。
- 本检查点只证明代码、schema、构建与既有通用 transport chain；没有运行 Wandering Sword/真实 UE 的 Watch mutation、ProcessEvent rate、nested call、disconnect/replay、慢消费者、drop/coalesce 或卸载 fixture。NET-015、WATCH-003、HOOK-003、HOOK-008 保持 `in_progress`，所有 UE profile 继续 `Not supported`。

### 0.29 R5.4 generation-bound bounded Hook scalar parameter capture

- 新增 `HookParameterCapturePlan`：`hook.add` 只从请求绑定的 exact immutable `ReflectedFunction` 编译计划，并将 function signature、parameter frame size、每个字段的 name/type/direction/kind/offset/size/bool mask 以及 enter/exit 顺序固化进 64-bit fingerprint。只有 `bool`、有符号/无符号整数、`float`、`double` 且 descriptor、宽度、offset、array dim、方向和总 payload 全部精确时可 admission；最多 64 个字段、单 phase 最多 512 bytes，单 phase 的字段名与类型名合计最多 8 KiB。FString/FText、Object/Weak/Soft、Struct、Array/Map/Set、delegate 以及其他需要构造/析构或引用语义的值稳定返回 `HOOK_CAPTURE_PLAN_UNAVAILABLE`，并带具体 `capture_error_code` 和参数名，不做降级。
- callback 只持有 immutable enabled snapshot/plan，在调用 original ProcessEvent 前复制 input/inout，在返回后复制 output/inout/return；payload 使用固定 16-byte `HCP1` header、phase、显式 status 与 plan fingerprint。拷贝目标是 callback 栈上的固定 512-byte buffer，字段读取受 SEH 边界保护；路径不分配、不锁服务 mutex、不遍历 live UObject、不解码、不序列化、不做 Pipe/磁盘 I/O。空 frame、不可读 frame、buffer/plan 异常分别编码为可排队的 `frame_unavailable`、`read_failed`、`output_too_small`/`invalid_plan` 状态，而不是伪造空参数。
...
- 本检查点执行 TypeScript/Vite production build并成功生成前端产物；推送后按仓库固定流程执行 Core Release build并成功生成 DLL。按用户要求未执行 lint、Vitest、component test、浏览器自动化或真实 UE/Wandering Sword 验证；因此这里只声明 R6 功能代码完成，不声明 `REFACTOR_PLAN.md` 的证据验收门通过，也不改变任何 UE support profile。

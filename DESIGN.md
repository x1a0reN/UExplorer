...
| R5 领域正确性 | 进行中（R5.1 flat Property read 代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情与显式 direct/inherited 分页；生产 reflection/type capture 从 exact ObjectSnapshot generation 采集完整结构以及 scalar/bool/FName/FString/FText/UObject/Weak/Soft flat descriptor；Main 发布 fingerprint-bound baseline PropertyCodec，实际开放 scalar/bool/FName/UObject；ObjectSnapshot 建立 immutable address index；`ObjectPropertyCommandService` 以 exact ObjectHandle + TypeSnapshot generation + declaring full path + exact property name + array index 入队，在游戏线程重新验证完整依赖并经 SafeMemory codec 读取；Rust Host 明确转发，React 从 snapshot/type metadata 构造请求；协议 schema/fixture 与 C++/Rust/React 构建已通过 | Wandering Sword 真实运行证据；FString/FText/Weak/Soft value-layout profile；nested Struct/Array/Map/Set descriptor、enum/bytecode、property write、GC/目标规模，以及 Memory/Call/World/Watch/Hook/Blueprint/Dump |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“可用”的既有功能包括：Watch/Hook producer、Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、Memory/Call/World/Blueprint/Dump 等尚未迁移的领域命令、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。
...
- `protocol/v1` 增加严格 request/output schema、Rust `deny_unknown_fields` 类型和 golden fixture；Rust `DomainService` 将十个 operation 明确路由到 Core；TypeScript client 只发送 exact path，`TypeBrowser`/`InspectorPane` 使用 generation-bound load-more，`Functions` 直接按完整函数路径查询并消费后端实现分类。Native/Blueprint 列表过滤在后端索引可用前明确禁用，不再按函数名猜测；UClass index 不再自动填成 instance target，Blueprint path 失败也不退回裸 index。真实 Call/实例选择尚未开放，不能把详情可读误写成函数可执行。

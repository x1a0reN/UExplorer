...
| R2 CoreRuntime/能力模型 | 进行中 | CoreRuntime 状态机、加密随机 session 与 request lease；一次性发布的 immutable EngineContext；offset validation report；依赖式 CapabilityRegistry；Runtime-owned GameThreadExecutor、IGameThreadPump/PostRender backend 与 thread/tick/stall 诊断；ShutdownCoordinator；集中式 SafeMemory 与可执行页写策略；稳定 Object/FunctionHandle 契约、生产 identity source 与执行点重验证；transport-neutral CoreCommandService 已承载 status 和 handle issue；VTable Hook RAII owner 与 callback quiet drain；危险 UObject 属性写及 index-only 调用入口已关闭 | identity source 目标 fixture、其余领域 command/Snapshot、剩余 Off/Settings 迁移 |
...
- `CapabilityRegistry` 通过显式 dependency graph 推导 capability，缺失依赖不会降级执行，未定义依赖和依赖环会使构建失败。`objects.handles` 只有在命令服务、生产 identity source 与活跃稳定的 game-thread executor 同时成立时才可用；`call.invoke` 仍因没有完成实际调用命令而保持 unavailable。未完成的 Snapshot、Watch、Hook collector、Blueprint 和完整 Dump fixture 同样保持 unavailable，而不是沿用旧页面的“已支持”表述。
...
- `ObjectHandle` 固化 `session_id + context_generation + index + serial + address + class_fingerprint`；`FunctionHandle` 额外绑定 serial-backed owner、canonical identity path 与 signature fingerprint。执行验证每次从 identity source 重读并逐字段比较；Harness 已覆盖 session/generation 过期、槽位复用、地址/类型变化及 function owner/path/signature 变化。生产 `ObjectArrayIdentitySource` 只接受 `epic_fuobjectitem_64_v1` 明确 profile：item size 必须为 `0x18/0x20`、object offset 必须为 `0`，且双读稳定、slot/InternalIndex 一致、cluster 合法并观察到正 serial；未知/加密布局不扫描猜值。对象与函数 identity 通过 `SafeMemory` 重读；函数身份路径由完整 outer 链的原始 FName comparison-index/number token 组成，不在执行边界调用未验证的显示名解码器，并绑定 serial-backed owner、反射 flags/size、exec pointer 后双读确认；所有读取拒绝非已观测 PostRender 线程。同步修正了 `index == Num()` 越界接收，以及 manual fixed/chunked 初始化未一致应用 object-table decrypt、fixed 初始化多解引用一级的问题。Handle 签发现已接入 game-thread 领域命令，`objects.handles` 按真实依赖动态报告；该 profile 仍待真实目标 fixture，实际函数调用命令尚未实现，因此旧 HTTP call 与前端执行控件继续返回/显示 `CALL_HANDLE_REQUIRED`。
...
R2 尚未完成：`FUObjectItem` identity source 的真实目标 profile fixture、其余 Object/Class/Enum/Memory/World/Call/Blueprint/Dump 领域命令、immutable Snapshot、Hook 热路径有界 collector/最后订阅卸载，以及剩余直接读取 `Off::*`/`Settings::*` 或遍历 live UE 容器的模块仍待迁移；这些完成前不会把 R2 标记为完成。

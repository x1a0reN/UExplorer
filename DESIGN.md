...
| R3 Named Pipe/Rust Host | 进行中 | 共享严格 RPC payload/limit 契约；C++/Rust 有界流式 framing；Core 已接入 current-user DACL、双方 PID 证据、4 个 joinable worker、256 pending 上限的 overlapped Named Pipe server，并跑通真实 Win32 Hello/Welcome、领域 Request/Response、deadline、ticket cancel、Ping/Pong 与精确 Shutdown 回执；Host `CoreRpcSession` 已实现完整关联状态机；FakeCore 共用同一类型 | Rust `CoreRpcClient`、EventHub、SessionManager、注入后 connected + ready 门、重连/事件/模糊测试 |
...
- UI 将 dll_loaded、already_loaded、failed 分开显示，DLL load 后不再自动采用 runtime.ini 或关闭弹窗。Core 侧 Pipe/Ready 边界已经建立，但 Rust `CoreRpcClient` 与注入后的握手等待尚未接入，因此当前仍不能把 DLL loaded 宣称为连接成功。
...
本阶段最新本地证据：VS2026 `Release|x64` Core 与 `/W4 /WX` harness 构建通过；Core harness 已通过真实本机 NPFS fixture，验证规范 PID 管道名、current-user-only DACL、服务端读取后客户端 SID 校验、客户端查询服务端 PID、严格握手、领域调用、reader/worker 并行取消、heartbeat、完整 Shutdown payload flush 与 joinable stop，同时覆盖稳定 Handle、严格 snapshot page、SafeMemory、PE/pattern/offset、生产 snapshot source 与 PostRender drain。Rust protocol 9 项、Tauri Host 19 项、FakeCore 5 项测试及三者 Clippy `-D warnings` 通过，其中覆盖 typed golden payload、协商上限、严格关联、deadline/cancel/late terminal、event seq/drop、shutdown acknowledgement、Host/FakeCore 完整 RPC 生命周期和 SnapshotCache。Named Pipe、平台安全、RPC session 及 offset discovery 契约通过，`npm run lint` 通过。Rust 真实 Pipe client/SessionManager、UE 目标的 FUObjectItem serial/name/snapshot、GWorld/GEngine candidate、GC churn、模块 LDR 遍历、最小化/加载期间 PostRender、Hook 恢复、注入超时、内存保护失败和完整目标生成 USMAP 语义验证仍是明确未执行项，不能用本机 harness 代替。
...
R3 仍未完成：Rust `CoreRpcClient` 的真实 Pipe reader/writer 与服务端 PID 检查、Host EventHub/subscriber backpressure、multi-PID `SessionManager`、Host 重连、注入后的 connected + Core Ready 门、事件与 frame fuzz/断线矩阵仍待实现。旧 HTTP 仍是 R4 前临时兼容路径，桌面链路尚未切到 Pipe，不能将 Core 侧 harness 描述为完整产品连通。

...
| R5 领域正确性 | 进行中（R5.1 Object/Type 查询、Codec 基础、反射与类型结构采集完成） | Object/Type 列表查询已复用 Host SnapshotIndex；worker-only `TypeCommandService` 已经由 Core/Named Pipe/Host 暴露 exact-path Class/Struct/Enum/Function 详情、显式 direct/inherited 成员页、直接子类页与 CDO 身份；PropertyCodec 具备有界值模型；ReflectionLayout 验证完整字段集和 UStruct 尺寸/对齐；生产 `ObjectSnapshotReflectionCandidateSource` 从单代 immutable Object Snapshot + SafeMemory 生成 UProperty/FProperty witness；生产 `ObjectSnapshotTypeCandidateSource` 再从 exact Object/Reflection generation 采集完整类型/函数结构、direct property/parameter、super/CDO 与 witnessed native exec；两条 capture 均由 Main 接入共享 PostRender scheduler，分帧复核、精确依赖封口并由 Worker publication/回收；Object Snapshot 逐条校验、分段存储并把旧代/失败工作集移交 Worker 回收；C++/Rust/schema/React 合成契约已通过 | 真实 UE profile 的 reflection/type 采集与详情查询证据、生产 PropertyCodec profile/capture、property descriptor/enum/bytecode 元数据、GC/目标规模，以及 Memory/Call/World/Watch/Hook/Blueprint/Dump 逐项验证 |

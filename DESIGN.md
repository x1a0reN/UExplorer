...
| R5 领域正确性 | 进行中（领域代码优先，真实目标验证后置） | 既有 Object/Type/property/call/World 与 stored/computed Actor transform 路径；strict Memory command；bounded Watch scheduler + pull drain；descriptor-proven enum codec + owned destructor journal；explicit-profile bounded Blueprint capture/disassembly；Hook collector/command、Dump coordinator/command 和 call.batch coordinator 原语 | 生产 UEnum table 与 Blueprint Script/profile witness；Hook ProcessEvent producer；Watch Pipe/Tauri push producer；Dump generator worker；call.batch adapter；复杂 UE 值生命周期与完整 transform mutation；全部真实 UE/Wandering Sword fixture |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“已由真实 UE 验证可用”的既有功能包括：R5.2 `call.invoke`、R5.3 全部 `world.*` 只读命令（含 reflected getter computed Actor world transform）、Memory、Watch、enum codec、Blueprint，以及尚未具备真实 producer/worker/adapter 的 Hook、Dump、call.batch 原语；还包括 Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、LocalPlayer/Pawn shortcut、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。
...
- Blueprint 路径现包含 exact FunctionHandle 与 generation-bound `BlueprintBytecodeCapture`、显式 immutable `BytecodeProfile`、bounded reader 和 fail-closed disassembly result。未知 opcode、截断、profile mismatch 都保留结构化失败/coverage 信息，不继续伪装完整 pseudocode。Main 尚未构造并发布 Script-array layout witness 或 bytecode profile source，因此 `blueprint.bytecode` 与 `blueprint.decompile` capability 仍为 false；Host/协议有严格 route/schema 不等于目标功能已可用。
- Hook 已有预分配有界 `HookEventCollector` 以及 add/list/enable/remove/log command/state 原语：热路径接口不做 JSON、Pipe I/O 或阻塞等待，enabled set 由 immutable snapshot 发布，并记录 drop/coalesce 诊断。add 与 disabled-to-enabled 现在还会取得 request lease，核对当前 Object/Type generation、完整 Function/Owner handles、display path 与 signature；producer lookup key 也覆盖完整 FunctionHandle 和 generations。但 release runtime 尚无事务式安装/恢复的 ProcessEvent producer/patch 接到 collector，故 `hook.monitor` 继续 false，不能只翻 capability probe。Dump 已有 single-active `DumpJobCoordinator` 和 start/list/get/cancel command 原语，定义 owned request、总 deadline、协作取消、有界 history 与 shutdown drain；没有实际 generator worker时所有 dump operation 都 fail closed，不再返回伪造的空 job store，前端页面也明确禁用。未来 worker 开放前还必须由 Host 约束 output root/canonical path，并让 job pin immutable snapshot/IR，而不是只保留 generation 数字。
- `FunctionCallBatchCoordinator` 已定义最多 64 项、总 request/result byte budget、单一总 deadline、逐项终态、fail-fast/continue policy、取消和 drain 所有权；新增 transport-neutral `FunctionCallBatchCommandService`，严格解析 submit/get/cancel/list、scope、exact handles 和 bounded JSON，并通过注入的 adapter 接口要求复用单目标安全调用路径。Main/Core/Host/协议尚未装配该 adapter/route，不能直接复制 ProcessEvent 路径，所以独立 `call.batch` capability 仍以 `CALL_BATCH_ADAPTER_NOT_READY` 保持 unavailable，不能借用已可用的 `call.invoke` 产生错误诊断。旧 `call.static` 也用独立 retired capability，静态调用只允许 exact CDO target 的 `call.invoke`。

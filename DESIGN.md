...
| R2 CoreRuntime/能力模型 | 进行中 | CoreRuntime 状态机、加密随机 session 与 request lease；一次性发布的 immutable EngineContext/identity/name-layout context；offset validation report；只依赖冻结 profile 与 SafeMemory 的 NamePool/Chunked NameArray 严格名称 codec；依赖式 CapabilityRegistry；Runtime-owned GameThreadExecutor、IGameThreadPump/PostRender backend 与 thread/tick/stall 诊断；ShutdownCoordinator；集中式 SafeMemory 与可执行页写策略；受边界校验的 x64 PE/LDR、pattern 与可读节版本探测；稳定 Object/FunctionHandle 契约、生产 identity source 与执行点重验证；`EngineFacade` 单一引擎边界、原子 immutable Snapshot store、生产 object/path metadata source 与 capture/validate/publish 三阶段有界 producer；Snapshot producer 已通过独占且可排空的 frame client 接入 PostRender；transport-neutral CoreCommandService 已承载 status 和 handle issue；VTable Hook RAII owner 与 callback quiet drain；危险 UObject 属性写及 index-only 调用入口已关闭 | identity/name/snapshot 的真实目标与 GC churn fixture、目标进程 LDR/版本 fixture、其余领域 command、剩余 Off/Settings 迁移 |
...
- `CoreHarness` 已覆盖 framing、1-byte 分片、加密随机 Core session、transport-neutral status/handle commands、Handle 输入拒绝/取消/lease drain、USMAP production writer/golden/独立解析、pattern 首个/跳过/末尾匹配、AMD64 PE 头与节边界、跨 64 KiB 窗口的可读节版本探测、跨页 no-access 范围、队列背压、GameThread owned work/ticket/超时/显式取消/C++ exception/SEH、生产 object/path snapshot metadata、空槽/读取失败区分、outer cycle、slot/count 变化拒绝、PostRender frame client 撤销与在途 drain、128 生产者容量、1000 次 HTTP connect/disconnect、占用端口无 fallback 和慢客户端 shutdown；Rust protocol 测试从同一 fixture 独立验证 USMAP 容器。`tests/contracts/verify-core-safety.ps1`、`tests/contracts/verify-core-runtime.ps1` 与 `tests/contracts/verify-platform-safety.ps1` 固化静态不变量。

本阶段最新本地证据：VS2026 `Release|x64` Core 与 `/W4 /WX` harness 构建通过，Core harness（含稳定 Handle、SafeMemory、pattern、合成 PE/版本探测、生产 snapshot metadata source 与 PostRender frame-client drain）与 Rust USMAP consumer 测试通过，平台安全契约通过，`npm run lint` 通过。真实 UE 目标的 FUObjectItem serial/name/snapshot、GC churn、模块 LDR 遍历、最小化/加载期间 PostRender、Hook 恢复、注入超时、内存保护失败和完整目标生成 USMAP 语义验证仍是明确未执行项，不能用本地 harness 代替。
...
R2 尚未完成：`FUObjectItem` identity/name/snapshot 的真实目标 profile 与 GC churn/大型对象表 fixture、目标进程 LDR/版本探测 fixture、其余 Object/Class/Enum/Memory/World/Call/Blueprint/Dump 领域命令、Hook 热路径有界 collector/最后订阅卸载，以及剩余直接读取 `Off::*`/`Settings::*`、裸指针或遍历 live UE 容器的模块仍待迁移；这些完成前不会把 R2 标记为完成。

...
| R5 领域正确性 | 进行中（R5.1 property read、R5.2 单目标 call、R5.3 immutable World 与 stored/computed Actor transform 代码已接入，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor、单层 `Array<flat>` descriptor、exact FVector/FRotator float/double descriptor 与 Windows x64 ScriptArray PropertyCodec；struct 从整值稳定快照解码；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame`，并只对 descriptor-proven canonical FVector/FRotator 开放 struct input/output；`WorldSnapshotCapture` 构建并复核 immutable World 关系；`WorldCommandService` 提供 worker-only 查询；`WorldTransformCommandService` 在一个 owned game-thread work 中用同一稳定 byte witness 读取 RootComponent 三个 `Relative*` 与三个 `bAbsolute*`，并在该 witness 内依次执行 exact `K2_GetActorLocation`、`K2_GetActorRotation`、`GetActorScale3D`，Host/React 将 stored 与 computed world transform 显式分离 | Wandering Sword 可执行文件/干净基线恢复及真实运行证据；World streaming/GC/目标规模；完整 witnessed UE setter 与 FHitResult 生命周期；任意 Struct 等复杂调用生命周期、enum input、batch job、真实 function round-trip、property write，以及 Memory/Watch/Hook/Blueprint/Dump |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“已由真实 UE 验证可用”的既有功能包括：R5.2 `call.invoke`、R5.3 全部 `world.*` 只读命令（含 reflected getter computed Actor world transform）、Watch/Hook producer、Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、Memory、LocalPlayer/Pawn shortcut、Blueprint/Dump 等尚未迁移或仅有 synthetic/code evidence 的领域能力、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。

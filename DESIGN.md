...
| R2 CoreRuntime/能力模型 | 进行中 | CoreRuntime 状态机与 request lease；一次性发布的 immutable EngineContext；offset validation report；依赖式 CapabilityRegistry；PostRender thread/tick/stall 诊断；ShutdownCoordinator；集中式 SafeMemory 与可执行页写策略；稳定 Object/FunctionHandle 契约与重验证；危险 UObject 属性写及 index-only 调用入口已关闭 | production serial identity source、Hook RAII、领域 command、剩余 Off/Settings 迁移与目标 fixture |
...
R2 尚未完成：production `FUObjectItem` serial layout 验证与 game-thread identity source、Handle 到领域执行命令的接入、Hook RAII owner、领域 command 边界和剩余直接读取 `Off::*`/`Settings::*` 或遍历 live UE 容器的模块仍待迁移；这些完成前不会把 R2 标记为完成。

...
| R5 领域正确性 | 进行中（R5.1 property read、R5.2 单目标 call、R5.3 只读 World 列表代码完成，真实目标验证待办） | Object/Type 列表复用 Host SnapshotIndex；worker-only `TypeCommandService` 暴露 exact-path 类型详情；生产 reflection/type capture 发布 flat descriptor 与 baseline PropertyCodec；`ObjectPropertyCommandService` 执行 exact-handle 属性读取；`FunctionCallCommandService` 以 exact ObjectHandle + FunctionHandle + TypeSnapshot generation 准备 owned `ParamFrame` 并在 witnessed game thread 调用 ProcessEvent；`WorldSnapshotCapture` 以 exact Object/Type generation 预筛候选，在共享 PostRender 预算内解析 GWorld、Actor typed outer 与 witnessed `ULevel.OwningWorld`，Worker 完成排序、计数和 publication，`WorldCommandService` 暴露 inspect/Level/Actor cursor page；Host 与 React 使用同一 DomainService/Named Pipe 主链路 | Wandering Sword 真实运行证据；World streaming/GC/目标规模；Actor detail/components/shortcuts/transform；复杂调用生命周期、enum input、batch job、真实 function round-trip、property write，以及 Memory/Watch/Hook/Blueprint/Dump |
| R6 前端状态重构 | 未开始 | UI lint 已清零，基础 Vitest 已建立 | session store、查询取消、BigInt 地址、真实能力 UI |
| R7 发布硬化 | 未开始 | 无 | 性能、压力、目标 fixture、文档和发布门全部通过 |

当前不能宣称“已由真实 UE 验证可用”的既有功能包括：R5.2 `call.invoke`、R5.3 `world.inspect/world.levels/world.actors.list`、Watch/Hook producer、Console、Actor Transform 写入、property write、baseline codec 之外的属性类型、Memory/World detail/shortcuts/transform、Blueprint/Dump 等尚未迁移的领域命令、尚未由目标进程生成并完成语义验证的完整 USMAP、未完成 capability 报告的 Offset，以及未经过真实目标进程卸载 fixture 的 Hook。未实现命令返回稳定 capability 错误，不会退回旧 HTTP。旧 HTTP/SSE/WS 源码未删除，但已从 release project 和运行入口隔离。

本机已有 `D:\Projects\UnrealEngine` 下 UE 4.21、4.24-4.27、5.0-5.4、5.6、5.7 源码，可用于建立候选布局、版本差异和行为语义证据，但源码本身不证明 Shipping binary 布局。`D:\Steam\steamapps\common\Wandering Sword` 已登记为后续真实 fixture：当前被动证据确认 x64 UE4/PhysX Shipping，第三方工件时间指向 4.26 系列；可执行文件同时存在 `.uedbg/.bind`、本地 `version.dll` 和既有注入日志，因此准确 engine profile 与干净基线必须由运行时 EngineContext、已加载模块和实际 frame/pipe trace 再确认。
...
- 新增 `Runtime/ParamFrame` 与 PropertyCodec owned-input encoder。参数帧使用 TypeSnapshot 的 exact `ParameterSize` 零初始化并拥有到请求终态；每个字段要求 supported、`array_dim=1`、descriptor size 与 reflected size 一致且范围完全落在帧内。当前生命周期边界只允许 bool、定宽整数、float/double、FName 和 UObject，其中输入 encoder 仅开放 bool、整数、float/double、UObject/null；数值做精确宽度与范围检查，复杂 UE 值不做猜测式构造或析构。
...
- `call.invoke` capability 现在依赖 property codec、TypeSnapshot、ObjectSnapshot、game-thread executor 和 FunctionHandle。Core Release、CoreHarness、Host route test、React lint/test/build 与协议 JSON 解析已通过；Harness 额外覆盖 owned scalar encode、descriptor width 拒绝和 ParamFrame 边界。尚未完成非平凡 FString/Text/Struct/container 生命周期、enum input、bounded batch job 和真实 UE fixture function round-trip，因此 R5.2 与 CALL-003/CALL-006/CALL-007 仍保持进行中，支持矩阵不变。
...
- Rust Host 只转发上述三个 operation；Actor detail/components/shortcuts 使用独立且明确 unavailable 的 `world.details` capability，transform 继续使用 `world.mutate`。React client 发送显式 cursor/filter，WorldBrowser 按 exact Level path 分组 Actor，并为 Level/Actor page 提供显式 load-more，不再把所有 Actor 重复渲染到每个 Level，也不把 detail capability failure 显示为“无组件”。
- Core Release 与 synthetic CoreHarness 能证明 immutable store、精确计数、过滤、stable Actor/Level handle、continuation cursor 和伪造末页 cursor 拒绝；它们不能证明真实 World/streaming/GC 行为。`D:\Steam\steamapps\common\Wandering Sword` 尚未启动或注入，本切片仍需在其运行时确认 EngineContext、GWorld、OwningWorld、streamed Level、GC churn 和卸载边界；Actor detail/components/shortcuts、FVector/FRotator/LWC codec 与 setter 继续保持 unavailable，支持矩阵不变。
